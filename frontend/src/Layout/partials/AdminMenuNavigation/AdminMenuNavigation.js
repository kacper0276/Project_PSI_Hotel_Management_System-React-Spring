      ...
      <div className="row flex-nowrap">
        <div className="col-auto col-md-3 col-xl-2 px-sm-2 px-0 bg-light">
          <div className="d-flex flex-column align-items-center align-items-sm-start px-3 pt-2 text-white min-vh-100">
            ...
            <nav className="nav nav-pills flex-column mb-sm-auto mb-0 align-items-center align-items-sm-start" id="menu">
              {['Ustawienia użytkowników', 'Ustawienia danych administratora', 'Dodaj nową ofertę pokoju', 'Edytuj oferty pokoi'].map((label, index) => (
                <button
                  key={index}
                  className={` align-middle px-0 d-flex justify-content-between ${activeBtn === index ? 'active btn btn-dark w-100' : 'btn btn-light w-100'}`}
                  onClick={() => changeActive(index)}
                  style={{ textAlign: 'left' }}
                >
                  <img src={icons[index]} alt={`Icon ${index + 1}`} className='icon-img' />
        ...
        <div className="col py-3">
          <div className="text-center">
            {activeBtn === null ? <h3>Panel administratora</h3> : panels[activeBtn]}
          </div>
        </div>
      </div>
    </div>
